...
这 2 小时会按产品经理的日常工作流往前走。前半段看方法和演示，后半段做案例回看和总结；完整 30 个 Tip 放在课后资料区，方便后续查阅。

| 顺序 | 内容 | 会看到什么 |
|---:|---|---|
| 1 | 开场与痛点共识 | 先聊大家现在用 AI 的真实场景和卡点 |
...
这一部分聚焦分享主线里的重点 Tip 和选讲 Tip。完整 30 个 Tip 索引保留在附录 D，补充 Tip 正文也放在附录 D，方便课后按场景查阅。

> 阅读方式：从这里开始按工作流顺序看。每个工作流先看目标和重点，再看对应的重点 Tip；补充 Tip 放在附录 D 作为资料库使用。

## 工作流一：接需求与资料整理

> 目标：把零散、口语化、不完整的业务信息，快速整理成结构化材料。
> 看完这一节，重点记住：先把输入材料整理成 AI 能理解的上下文，后面的输出才更稳定。
> 这一节主要看：为什么要先整理会议纪要、聊天记录和业务反馈，不急着进入 PRD。
> 演示看点：展示一段会议纪要 / 聊天记录如何整理成 Markdown，重点看信息结构化过程。

---

#### 更多可查

本工作流覆盖 Tip 1-4。正文聚焦“先整理上下文”这个原则，完整方法见附录 D。

## 工作流二：需求澄清与范围收敛

> 目标：让 AI 帮助 PM 把需求问清楚、想完整、砍到能落地。
> 看完这一节，重点记住：遇到模糊需求时，不急着写方案，先反问、评分、收敛范围。
> 这一节主要看：Tip 5 → Tip 6 → Tip 7。Tip 8 作为课后资料。
> 演示看点：用同一个模糊需求连续演示反问关键问题、完整度评分、MVP 范围收敛。
...
本工作流的补充方法见附录 D，重点可以先看 Tip 8。

## 工作流三：PRD 与评审准备

> 目标：让 AI 辅助产品经理把 PRD 写得更有结构，但最终判断仍由产品经理负责。
> 看完这一节，重点记住：AI 可以先起草，但还要反向检查，最终判断仍然在产品经理手里。
> 这一节主要看：Tip 9 和 Tip 11，先生成草稿，再反向检查。
> 演示看点：沿用前面整理出的需求材料，生成一段 PRD 草稿，再让 AI 检查问题和边界。
...
本工作流覆盖 Tip 9-13。正文聚焦 PRD 草稿和反向检查，其他方法见附录 D。

## 工作流四：HTML 高保真原型

> 目标：把抽象需求快速变成可点击、可讨论、可评审的界面。
> 看完这一节，重点记住：页面型需求可以先做可点击原型，让讨论从文字想象变成界面共识。
> 这一节主要看：Tip 14，先把抽象需求变成 HTML 高保真原型。
> 演示看点：用 Tip 14 的 Prompt 展示一个 HTML 原型结果，重点看需求如何变成可讨论界面，以及后续如何迭代。
...
本工作流覆盖 Tip 14-21。正文聚焦 HTML 原型生成，迭代、预审、发布和反向生成文档的方法见附录 D。

## 工作流五：协同交付与上线

> 目标：让 AI 帮助 PM 更好地和研发、测试、业务方协同，而不是只停留在写文档。
> 看完这一节，重点记住：看起来很小的变更，也可以先用 AI 扫一遍页面、接口、权限、数据和测试影响。
> 这一节主要看：Tip 22，让大家看到一个小变更背后可能影响页面、接口、权限、数据和测试。
> 演示看点：用“顺便加一个导出功能吧”做输入，让 AI 输出变更影响分析。
...
本工作流覆盖 Tip 22-26。正文聚焦需求变更影响分析，研发任务、测试关注点、上线公告和向上汇报方法见附录 D。
...
> 看完这一节，重点记住：AI 可以辅助看数和写 SQL，但口径、权限和结论都需要人工确认。
> 这一节主要看：Tip 27 / Tip 28，重点放在数据口径、权限范围和结论校验。
> 演示看点：用样例 CSV 或表结构演示 AI 如何辅助分析和写 SQL，同时强调结果需要人工确认。
...
本工作流覆盖 Tip 27-30。正文聚焦数据分析与 SQL，趋势分析和项目复盘方法见附录 D。

---

# Part 2：真实案例闭环回看

前面的案例已经穿插在 Tip 5-7、Tip 9/11、Tip 14、Tip 22 后面。这里用 5-7 分钟把链路收回来。

这一节重点看一件事：

> 前面的 Tip 不是散点技巧，而是可以连成一条从需求到交付的工作链路。

## AI 商品视频系统：从需求到原型到文档

这个案例不是为了重点讲 AI 商品视频系统本身，而是回看一个产品经理如何用 AI 完成从需求接入、原型迭代、评审对齐到文档交付的全过程。
...
传统商品视频制作依赖人工拍摄和剪辑，周期长、成本高、产出慢。因此我们希望探索一个轻量级的 AI 商品视频系统：
...
| 阶段 | 对应 Tip | 产物 | 回看重点 |
|---|---|---|---|
| 接到一句话需求 | Tip 5 | 关键问题清单 | 不急着写方案，先问清楚 |
| 判断需求成熟度 | Tip 6 | 完整度评分和缺口清单 | 哪些信息不确认会影响开发 |
| 收敛一期范围 | Tip 7 | MVP 范围表 | 先保证一个能落地的闭环 |
| 生成 PRD 草稿 | Tip 9 | PRD 初稿 | AI 先搭结构，PM 再判断 |
| 反向检查 PRD | Tip 11 | 风险和待确认问题 | 让 AI 写草稿，也让 AI 挑问题 |
| 生成 HTML 原型 | Tip 14 | 任务创建页面原型 | 把抽象需求变成可讨论界面 |
| 扫描变更影响 | Tip 22 | 影响面分析 | 小改动也要看页面、接口、权限、测试 |
| 看数和复盘 | Tip 27 / 28 / 30 | 数据分析和复盘框架 | 口径和结论仍需人工确认 |
...
1. 一开始的需求通常很模糊，不要直接写方案，先让 AI 帮我们反问问题。
2. 需求问清后，再用 AI 做完整度评分和 MVP 收敛，避免一期范围过大。
3. 页面型需求可以先生成 HTML 原型，让业务方和研发直接看界面。
4. 需求变更和上线复盘阶段，AI 可以辅助扫影响面、补材料、沉淀经验。

一句话总结：

> AI 商品视频系统只是例子，真正可复用的是这条链路：澄清 → 收敛 → 原型 → 评审 → 变更 → 复盘。

---

# Part 3：进阶能力展望

> 这一段做方向性展望，目的是让大家知道后面可以继续关注什么，点到为止。

> 前面讲的是大家可以先尝试的基础工作流。
> 接下来这些能力属于后续进阶方向，先建立概念。
...
> 这一节用于收尾。后面的 Prompt 清单和附录主要用于课后查阅。
...
2. 再看“简短演示”：Tip 27、28。
3. 遇到具体工作场景时，再到附录 D 里按工作流查完整 30 个 Tip。
4. 需要复制模板时，直接到 Part 5 找对应 Prompt。

> 结尾回到 30 个 Tip 总览时，把它作为课后索引介绍。
...
> 到这里，分享主线结束。后面的 Prompt 清单和附录主要用于课后查阅。

---

# Part 5：课后可参考的 Prompt 清单

> 这部分是课后资料区，大家可以按场景复制使用。
...
30 个 Tip 是课后资料库。分享结尾回到这张表时，把它作为课后索引介绍；后续优先参考“现场重点”“简短演示”和“课后可试”三类。
...
以下是分享主线没有展开的 Tip 正文，主要用于课后按场景查阅。重点 Tip 的正文保留在 Part 1。
